...
class OrganizationCRUD:
    ...
    async def get_organization_by_id(
        ...
        if org and include_members:
            ...
            roles_query = (
                ...
                .join(Role, user_roles.c.role_id == Role.id)
                .where(
                    user_roles.c.org_id == org_id, user_roles.c.user_id.in_(user_ids)
                )
            )
    ...
    async def transfer_organization_ownership(
        ...
        is_member = await OrganizationCRUD.is_organization_member(
            db, new_owner_id, org_id
        )
        if not is_member:
            await OrganizationCRUD.add_user_to_organization(db, new_owner_id, org_id)
